...
rownames(Metabolite.data)=Metabolite.data$Matched.Compound
Metabolite.data$compoundID <- NULL
...
filtered.classes <- class.sums[rowSums(class.sums[2:7])>200,]
...
combinations.merge <- combinations %>% rename(Metabolite = 1,
                                        Class = 2)
#combinations[] <- lapply(combinations.merge, function(x) paste("joined.data$", x, sep=""))
combinations <- t(combinations.merge)
combinations <- data.frame(combinations)
rownames(combinations) <- c("1","2")
